...
def query_bedrock(
    message, session_id="", model_params=None, context="", conversation_history=None
):
    """
...
def check_password():
    ...
    def password_entered():
        ...
        if hmac.compare_digest(
            st.session_state["username"].strip(), "admin"
        ) and hmac.compare_digest(st.session_state["password"].strip(), "admin123"):
            ...
            print(
                f"DEBUG LOGIN: Comparação - Usuário igual: {st.session_state['username'].strip() == 'admin'}"
            )
            print(
                f"DEBUG LOGIN: Comparação - Senha igual: {st.session_state['password'].strip() == 'admin123'}"
            )
            ...
            if st.session_state.uploaded_file is not None:
                ...
                if file_type == "PDF":
                    return format_context(
                        read_pdf_from_uploaded_file(file),
                        f"Contexto do arquivo PDF: {file.name}",
                    )
                elif file_type == "TXT":
                    return format_context(
                        read_txt_from_uploaded_file(file),
                        f"Contexto do arquivo TXT: {file.name}",
                    )
                elif file_type == "CSV":
                    return format_context(
                        read_csv_from_uploaded_file(file),
                        f"Contexto do arquivo CSV: {file.name}",
                    )

        elif st.session_state.rag_source == "Texto Direto":
            if st.session_state.direct_text:
                return format_context(
                    st.session_state.direct_text, "Contexto do Usuário"
                )
        ...
        if not is_duplicate:
            ...
            if attached_file is not None:
                ...
                st.session_state.messages.append(
                    {
                        "role": "user",
                        "content": user_message_with_attachment,
                        "time": timestamp,
                    }
                )
            else:
                st.session_state.messages.append(
                    {"role": "user", "content": user_message, "time": timestamp}
                )
            ...
            with st.chat_message("assistant", avatar=logo_path):
                ...
                with st.spinner():
                    current_session_id = (
                        "" if is_first_message else st.session_state.session_id
                    )

                    rag_context = get_rag_context()

                    if file_content:
                        file_context = format_context(
                            file_content,
                            f"Conteúdo do arquivo anexado ({attached_file.name})",
                        )
                        if rag_context:
                ...
                if result:
                    ...
                    if "sessionId" in result:
                        ...
                        if st.session_state.current_chat_index < len(
                            ...
                            print(
                                f"DEBUG: Histórico atualizado com session_id '{new_session_id}'"
                            )
...
def extract_title_from_response(response_text):
    ...
    sentence = (
        sentences[1] if len(sentences) > 1 and len(sentences[0]) < 15 else sentences[0]
    )
...
def regenerate_message(index):
    ...
    if (
        index < 0
        or index >= len(st.session_state.messages)
        or st.session_state.messages[index]["role"] != "user"
    ):
        return
    ...
    if result:
        ...
        if (
            index + 1 < len(st.session_state.messages)
            ...
            st.session_state.messages.insert(
                index + 1,
                {"role": "assistant", "content": new_response, "time": timestamp},
            )
    else:
        status_placeholder.error(
            "Não foi possível regenerar a resposta. Por favor, tente novamente."
        )
        time.sleep(2)
...
def edit_message(index, new_content):
    ...
    st.session_state.messages[index]["time"] = (
        datetime.now().strftime("%H:%M") + " (editada)"
    )

    if st.session_state.messages[index]["role"] == "user" and index + 1 < len(
...
def create_new_chat():
    ...
    st.session_state.chat_title = (
        f"Nova Conversa ({datetime.now().strftime('%d/%m/%Y')})"
    )

    st.session_state.chat_history.append(
        {"id": "", "title": st.session_state.chat_title, "messages": []}
    )
...
def handle_message_if_content():
    ...
    if st.session_state.user_input and st.session_state.user_input.strip():
        print(
            f"DEBUG: handle_message_if_content acionado com: '{st.session_state.user_input}'"
        )
...
if check_password():
    ...
    with main_col2:
        ...
        if st.session_state.renaming:
            ...
            with col1:
                st.markdown(
                    f'<div class="chat-title">{st.session_state.chat_title}</div>',
                    unsafe_allow_html=True,
                )
            with col2:
        ...
        with messages_container:
            for idx, message in enumerate(st.session_state.messages):
                if st.session_state.editing_message == idx:
                    st.markdown(
                        '<div class="edit-message-container">', unsafe_allow_html=True
                    )
                    st.text_area(
                        "Editar mensagem",
                    ...
                    with st.chat_message("user"):
                        ...
                        st.markdown(
                            '<div class="message-actions">', unsafe_allow_html=True
                        )
                        col1, col2 = st.columns([1, 1])
                        with col1:
                        ...
                        with col2:
                            if (
                                idx + 1 < len(st.session_state.messages)
